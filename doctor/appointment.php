...
<center>
   <p>
   <h6>Welcome <strong><?php echo $_SESSION["name"]; ?></h6>
   </p> 
</center>
<div class="container">
<?php
...
book_appointment.appointment_start, 
book_appointment.appointment_end, 
book_appointment.reason
FROM book_appointment 
INNER JOIN paitent
ON book_appointment.from_paitent_id=paitent.id 
INNER JOIN doctor
ON book_appointment.to_doctor_id=doctor.id WHERE book_appointment.status=1 && to_doctor_id=".$_SESSION['id'];
...
<?php	
   $i++;
	  ...
	  echo '<center><mark style="color:red;">Opps! There are no accepted appointments.</mark></center>';
   }
   ?>
</div>
<br>
