...
<input type="submit" name="btn1" class="btn btn-success" style="align:center;display:grid; margin:auto;" value="Submit"> 
...
if (isset($_POST['btn1']))
		{
			...
			if(!empty($_POST['speacility'])){
				if(empty($_POST['location']))
			  $x .= " category='$speciality'";
				else
			  $x .= " AND category='$speciality'";
			}
			if(!empty($_POST['search'])){
				if(empty($_POST['location']) && empty($_POST['speacility']))
					$x .= " CONCAT(name,city,hospitaladress,fees,qualification) LIKE '%$search%'";
				else
					$x .= " AND CONCAT(name,city,hospitaladress,fees,qualification) LIKE '%$search%'";
			}
			
			 $sql = $query.''.$where.''.$x;
			//echo $sql;
			/*if($add!="" || $speacility!="" || $search!=""){
			$sql="SELECT * FROM doctor WHERE 
			city='$add' && category='$speciality' 
			||
